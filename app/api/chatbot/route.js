...
if (meetingState.step === "awaiting_auth_confirmation") {
  ...
  if (err.message.includes("not authenticated")) {
    return `🔒 Please authenticate first by visiting: https://slackbot-gemini.vercel.app/api/auth/redirect?email=${meetingState.email}`;
  }
  throw err;
});
  ...
  try {
    ...
    if (meetingState.step === "time") {
      ...
      if (check.available) {
        ...
        // --- START OF CHANGES ---
        let meetLink = 'Not generated. Please check the calendar.'; // Default link text
        try {
          // 1. Capture the returned object
          const eventDetails = await createGoogleMeetEvent({
              name: meetingState.name,
              email: meetingState.email,
              date: meetingState.date,
              time: meetingState.time,
          });
          // 2. Extract the link if it exists
          if (eventDetails && eventDetails.meetLink) {
        ...
        // 3. Add the link to the summary message
        const summary = `✅ Meeting Scheduled!
