...
export const getGoogleTokens = async (email) => {
  // 🛠 Clean Slack's email formatting if present
  const cleanEmail = email.includes('|') ? email.split('|')[1].replace('>', '') : email;

  console.log("🔍 Fetching tokens for:", cleanEmail); // Step 1 log

  const tokenData = await redis.get(`tokens:${cleanEmail}`);
  console.log("📦 Redis response:", tokenData); // Step 2 log

  if (!tokenData) {
    console.error("❌ No token found in Redis for this user:", cleanEmail);
    throw new Error("No token found for user");
  }
  ...
  if (Date.now() >= expires_at) {
    ...
    await redis.set(`tokens:${cleanEmail}`, JSON.stringify({ access_token, refresh_token, expires_at }));
    console.log("✅ Token refreshed and saved back to Redis.");
  }
