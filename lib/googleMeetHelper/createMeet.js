...
// --- CONFIGURATION ---
const CALENDAR_ID = 'hisaan.sakhawat@gmail.com'; // Or your specific calendar ID
const SCOPES = ['https://www.googleapis.com/auth/calendar'];
// --- END CONFIGURATION ---
...
export const createGoogleMeetEvent = async ({ name, email, date, time }) => {
    try {
       ...
       // return createdEvent.data;
        ...
        console.log('✅ Google Calendar event created:', createdEvent.data.htmlLink);
        console.log('✅ Google Meet Link:', meetLink); // Log the direct Meet link

        // You can now return the whole event or just the parts you need
        return {
...
export const createGoogleMeetEvent = async ({ name, email, date, time }) => {
    try {
        ...
        const event = {
            ...
            // DO NOT ADD attendees here for a personal @gmail.com account
            reminders: {
                useDefault: false,
                overrides: [
                    // You can keep reminders for the calendar owner
                    { method: 'popup', minutes: 15 },
                ],
            },
            conferenceData: {
                createRequest: {
                    requestId: `meeting-${name}-${Date.now()}`,
                    conferenceSolutionKey: {
                        type: 'hangoutsMeet' // This is the correct value
                    }
                }
            },
        };
        ...
        const createdEvent = await calendar.events.insert({
            ...
            resource: event,
            conferenceDataVersion: 1,
            // DO NOT sendNotifications here
        });

        // This will now work because the event can be created successfully
        const meetLink = createdEvent.data.hangoutLink;
        ...
        // This will now catch the original 'Invalid conference type' error if it persists,
        // or other issues.
        console.error('❌ Error creating Google Calendar event:', error);
        throw new Error('Failed to create Google Calendar event.');
    }
